...
kernel8.img: $(BUILD_DIR)/boot.o $(OFILES)
	...
	aarch64-none-elf-objcopy -O binary $(BUILD_DIR)/kernel8.elf $(IMG_DIR)/kernel8.img

clean:
	del *.img .\object\kernel8.elf .\object\*.o

run:
	qemu-system-aarch64 -M raspi3 -kernel $(IMG_DIR)/kernel8.img -serial null -serial stdio
